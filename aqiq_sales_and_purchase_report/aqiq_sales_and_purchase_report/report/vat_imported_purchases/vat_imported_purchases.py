...
def execute(filters=None):
    ...
    for d in query_data:
        ...
        row["custom_tims_cu_no"] = d.get("custom_tims_cu_no")
...
def get_columns():
    ...
    columns = [
        ...
        },
        {
            ...
            "label": _("custom_description"),
            "fieldtype": "Data",
            "options": "Purchase Invoice",
            "width": 150,
        },
        {
            "fieldname": "custom_tims_cu_no",
            "label": _("TIMS CU No"),
            "fieldtype": "Data",
            "width": 150,
        },
...
def get_data(conditions):
    ...
    data = frappe.db.sql(
        """select s.custom_tims_cu_no as custom_tims_cu_no, s.bill_no as supp_inv, s.bill_date as supp_inv_date, s.custom_shipment_no as shipment_no, c.custom_local_and_international as local_and_international, c.tax_id as pin_no, s.supplier as supplier, s.posting_date as posting_date, s.name as name,
			...
			where s.docstatus=1 and c.custom_unregistered=0 and c.custom_import_vat=1 {} order by s.posting_date DESC""".format(
            conditions
        ),
        as_dict=1,
    )
