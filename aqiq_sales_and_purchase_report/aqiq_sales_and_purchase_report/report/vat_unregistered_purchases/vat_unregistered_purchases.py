...
def get_data(conditions):
    ...
    # 		where s.docstatus=1 and c.custom_unregistered=1 {} group by s.Supplier order by 1""".format(conditions), as_dict=1)
    ...
    data = frappe.db.sql(
        """select s.supplier_name as supplier_name, SUM(i.base_net_amount) as total 
			...
			inner join `tabSupplier` c on s.supplier = c.name
			where s.docstatus=1 and c.custom_unregistered=1 and i.item_tax_template NOT IN ('Zero Rated - IG', 'Zero Rated - IAL','Exempt - IG','Exempt - IAL') {} group by s.supplier order by 1""".format(
            conditions
        ),
        as_dict=1,
    )
