			...
			if not filters.get("level_2") and not filters.get("level_3") and  not filters.get("value") and not filters.get("range"):
				...
				for i in level_1:
					...
					for b in level_1_child:
						tax_amount = b.get("amount_exclusive")*(b.get("custom_total_tax_percentage")/100) or 0.0
						row = frappe._dict({
						...
						"custom_tax_type" : b.get("custom_tax_type") or "",
						"custom_total_tax_percentage" : b.get("custom_total_tax_percentage") or 0.0,
				...
				for i in level_1:
					...
					for b in level_2:
						...
						for c in level_2_child:
							tax_amount = c.get("amount_exclusive")*(c.get("custom_total_tax_percentage")/100) or 0.0
							...
							"custom_tax_type" : c.get("custom_tax_type") or "",
							"custom_total_tax_percentage" : c.get("custom_total_tax_percentage") or 0.0,
					...
					for b in level_2:
						...
						for c in level_3:
							...
							for d in level_3_child:
								tax_amount = d.get("amount_exclusive")*(d.get("custom_total_tax_percentage")/100) or 0.0
								...
								"custom_tax_type" : d.get("custom_tax_type") or "",
								"custom_total_tax_percentage" : d.get("custom_total_tax_percentage") or 0.0,
...
def group_level_1_1(conditions = "" , grouping_level_1 = None,  grouping_level_value  = None, range_by = None , filters = None):
	if grouping_level_1:
		...
		data =  frappe.db.sql(""" select  {} as level_1,  s.posting_date, s.name  
			...
			s.custom_tax_type ,  i.custom_total_tax_percentage ,  i.item_group ,
...
def get_data_normal(conditions   = "", filters = ""):
		...
		query =  frappe.db.sql(""" select  s.posting_date, s.name as voucher_no, s.currency as currency, i.item_code ,  i.item_name ,   i.qty,  s.status ,   i.amount as amount ,
			...
			s.project ,  s.cost_center as division, s.custom_department as department,
			YEAR(s.posting_date) as year  , s.custom_tax_type ,  i.custom_total_tax_percentage  , s.status
		...
		for d in query:
			...
			row["custom_tax_type"] = d.get("custom_tax_type") or ""
			row["custom_total_tax_percentage"] = d.get("custom_total_tax_percentage") or 0.0
			...
			tax_amount = row["amount_exclusive"]*(d.get("custom_total_tax_percentage")/100) or 0.0
	...
	if not filters.range:
		...
		if not only_summary:
			columns.extend(
				...
				{
					"fieldname": "custom_total_tax_percentage",
