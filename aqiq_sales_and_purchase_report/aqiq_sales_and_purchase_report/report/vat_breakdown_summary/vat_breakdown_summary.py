...
def execute(filters=None):
    ...
    # 		where s.docstatus=1 and i.custom_total_tax_percentage= '16%' {} group by s.cost_center order by 1""".format(conditions), as_dict=1)
	...
	#  	from `tabSales Invoice` s 
	# 	inner join `tabSales Invoice Item` i on s.name = i.parent
	# 	inner join `tabCustomer` c on s.customer = c.name and c.custom_unregistered=0
    ...
    registered_sales = frappe.db.sql(
        ...
        inner join `tabSales Invoice Item` i on s.name = i.parent
        inner join `tabCustomer` c on s.customer = c.name
        where s.docstatus=1 and c.custom_unregistered=0 and i.custom_total_tax_percentage='16' {} group by s.cost_center order by 1""".format(
            conditions
        ),
        as_dict=1,
    )
    ...
    # custom_unregistered_sales = frappe.db.sql(
    #     """select s.cost_center as division, SUM(i.base_net_amount) as base_net_amount 
	#  	from `tabSales Invoice` s 
	# 	inner join `tabSales Invoice Item` i on s.name = i.parent
	# 	inner join `tabCustomer` c on s.customer = c.name and c.custom_unregistered=1
	# 	where s.docstatus=1 and i.item_tax_template NOT IN ('Zero Rated - IG', 'Zero Rated - IAL','Exempt - IG','Exempt - IAL') {} group by s.cost_center order by 1""".format(
    #         conditions
    #     ),
    #     as_dict=1,
    ...
    custom_unregistered_sales = frappe.db.sql(
        ...
        inner join `tabSales Invoice Item` i on s.name = i.parent
        inner join `tabCustomer` c on s.customer = c.name
        where s.docstatus=1 and c.custom_unregistered=1 and i.custom_total_tax_percentage='16' {} group by s.cost_center order by 1""".format(
            conditions
        ),
        as_dict=1,
    )
    ...
    exports_sales = frappe.db.sql(
        """select s.cost_center as division, SUM(i.base_net_amount) as base_net_amount 
			...
			inner join `tabSales Invoice Item` i on s.name = i.parent
			inner join `tabCustomer` c on s.customer = c.name
			where s.docstatus=1 and c.custom_unregistered=0 and i.item_tax_template IN ('Zero Rated - IG', 'Zero Rated - IAL' ) {} group by s.cost_center order by 1""".format(
            conditions
        ),
        as_dict=1,
    )
    ...
    exempt_sales = frappe.db.sql(
        """select s.cost_center as division, SUM(i.base_net_amount) as base_net_amount 
			...
			inner join `tabSales Invoice Item` i on s.name = i.parent
			inner join `tabCustomer` c on s.customer = c.name
			where s.docstatus=1 and c.custom_unregistered=0 and i.item_tax_template IN ('Exempt - IG','Exempt - IAL') {} group by s.cost_center order by 1""".format(
            conditions
        ),
        as_dict=1,
    )
    ...
    gen_purchases = frappe.db.sql(
        """select s.cost_center as division, SUM(i.base_net_amount) as base_net_amount 
			...
			where s.docstatus=1 and c.custom_local_and_international='Local' and c.custom_unregistered=0 and c.custom_import_vat=0 and i.custom_total_tax_percentage= '16%' {} group by i.cost_center order by 1""".format(
            conditions
        ),
        as_dict=1,
        debug=True,
    ...
    custom_import_vat_purchases = frappe.db.sql(
        """select s.cost_center as division, SUM(s.taxes_and_charges_added) as base_net_amount
		 	from `tabPurchase Invoice` s 
			inner join `tabSupplier` c on s.supplier = c.name
			where s.docstatus=1 and c.custom_unregistered=0 and c.custom_import_vat=1 {} group by s.cost_center order by 1""".format(
            conditions
        ),
        as_dict=1,
    )
    ...
    # custom_import_vat_purchases = frappe.db.sql("""select s.cost_center as division, s.bill_no as supp_inv, s.bill_date as supp_inv_date, s.shipment_no as shipment_no, c.custom_local_and_international as custom_local_and_international, c.tax_id as pin_no, s.supplier as supplier, s.posting_date as posting_date, s.name as name,
    ...
    # 		inner join `tabPurchase Invoice Item` i on s.name = i.parent
    # 		inner join `tabSupplier` c on s.supplier = c.name
    # 		where s.docstatus=1 and c.custom_unregistered=0 and c.custom_import_vat=1 {} group by s.cost_center order by s.posting_date DESC""".format(conditions), as_dict=1)
    ...
    totals2ur = 0.0
    rowur2 = {}
    rowur2["details"] = "custom_unregistered Sales"
    for d in custom_unregistered_sales:
    ...
    totals10 = 0.0
    row10 = {}
    row10["details"] = "custom_import_vats"

    for d in custom_import_vat_purchases:
        # if d.is_return == 1:
