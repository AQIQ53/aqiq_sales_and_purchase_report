...
def get_data(conditions):
    ...
    data = frappe.db.sql(
        """select c.tax_id as pin_no, s.customer_name as customer_name, s.posting_date as posting_date, s.name as name,
			...
			where s.docstatus=1 and c.custom_unregistered=0 and i.item_tax_template IN ('Exempt - IG','Exempt - IAL') {} group by s.name order by s.posting_date DESC""".format(
            conditions
        ),
        as_dict=1,
    )
